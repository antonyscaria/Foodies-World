<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			$food 		= htmlentities($_POST['chkfood'], ENT_QUOTES, 'UTF-8');
			$price 		= htmlentities($_POST['chkprice'], ENT_QUOTES, 'UTF-8');
			$cus_id=$_SESSION['userid'];
			if($name != "" && $addr != "" && $email != "" && $phone != "" && $food != "" && $price != "") {
				
				$insert = $db->query("INSERT INTO basket(customer_name, contact_number, address, email, total, status, date_made,cus_id) VALUES('".$name."', '".$phone."', '".$addr."', '".$email."', '".$price."', 'pending', NOW(),'".$cus_id."')");
		...
		if(isset($_POST['item_id_qty']) && $_POST['item_id_qty'] != "") {
