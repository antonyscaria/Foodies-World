  ...
  if(!isset($_SESSION['user'])) {
    ...
    $msg = "";
        ...
        if(isset($_GET['uid']))
        {
          ...
          if($edit != "") {
              ...
              $sql = $db->query("SELECT * FROM `food`WHERE id='".$edit."'");
              $row = $sql->fetch_assoc();
              $id = $row['id'];
              ...
              $image_url = "../image/FoodPics/$id.jpg";

              ?>
             <script> document.getElementByName("category").value=<?php echo $foodDesc;?></script>
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        if(isset($_POST['submit']) && isset($_FILES['file'])) {
            $id=htmlentities($_POST['fid'], ENT_QUOTES, 'UTF-8');
            ...
            if($name != "" && $price != "" && $desc != "" && empty($file) == false) {
                
                $ext = explode(".", $_FILES['file']['name']);
                
                if(in_array($ext[1], $allowed_ext)) {
                    
                        $insert = $db->query("UPDATE `food` SET `food_name`='$name',`food_price`= '$price',`food_description`='$desc' WHERE `id`='$id'");
                        if($insert) 
                        {
...
<body>
    ...
    <form method="post" action="foodupdate2.php" enctype="multipart/form-data">   
    <div class="container">
      <h1>Update Food</h1>
      <hr>
      <?php echo $msg; ?>
      ...
      <input type="text" name="name" id='name' placeholder="Food Name" value="<?php echo $foodName; ?>"required ><br>
      <input type="text" name="price" id='price' placeholder="Price" value="<?php echo $foodPrice; ?>" required ><br>
      <input type="text" name="desc" id='desc' placeholder="Description" value="<?php echo $foodDesc; ?>" required ><br>
      <label style="display: block; border-radius: 5px; letter-spacing: 2px; background: #fff; color: #333; padding: 10px; border: 1px solid #ccc; cursor: pointer; text-align: center; font-size: 15px; font-weight: bold;" for="file" class="file_lbl"><i style="font-weight: bold; font-size: 19px;" class="pe-7s-upload"></i><?php echo $image_url; ?><input type="file"  style="display: none;" id="file" name="file" required /></label>
      <input type="submit" id="submit" class="registerbtn" name="submit"  onclick="return valdate2()" value="Update">
    </div>
